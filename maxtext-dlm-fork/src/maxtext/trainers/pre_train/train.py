...
def loss_fn(model, config, data, dropout_rng, params, sparsity_state=None, is_train=True):
  ...
  """
  # Phase 23 — MDLM objective branch: noise the inputs in-place before the
  # model.apply call. The AR cross-entropy code below sees `data["inputs"]`
  # already replaced with the masked view; we then overwrite `xent_sum`
  # post-forward with the MDLM loss formula. See `maxtext.diffusion`.
  # Phase — SASD objective branch (Fast-dDrive Section-Aware Stochastic Diffusion).
  # The grain SASD loader yields a DOUBLED-sequence batch with extra higher-rank keys
  # (cos/sin/attn_mask/labels_final/...). We MUST run before the generic [B,L] data
  # decimation/slicing below: pop the SASD-only keys, flatten the 2 doubled rows into
  # the batch axis ([2B, 2L]), inject 3D M-RoPE cos/sin + the hybrid block-causal 4D
  # mask via attention_metadata, run a single decoder pass, then compute
  # section_weighted_ce (noisy half) + causal_ce (clean half) and stuff the global
  # numerator into xent_sum so the downstream division by total_weights matches the
  # NNX global loss sum_b(sec+causal)/sum_b(num_items). See maxtext.diffusion.sasd.
  is_sasd = bool(getattr(config, "objective", "ar") == "sasd")
  if is_sasd:
    from maxtext.diffusion.sasd import sasd_loss_from_logits

    # B and L from STATIC shapes (jit-safe). data["inputs"] is the doubled [2B, 2L]; under jit,
    # int(data["sasd_B"][0]) is a traced array -> ConcretizationTypeError, so derive from shapes.
    sasd_B = data["inputs"].shape[0] // 2
    sasd_L = data["inputs"].shape[1] // 2
    sasd_labels_final = data["sasd_labels_final"]
    sasd_original_labels = data["sasd_original_labels"]
    sasd_weights = data["sasd_weights"]
    sasd_num_items = data["sasd_num_items"]
    sasd_cos = data["sasd_cos"]
    sasd_sin = data["sasd_sin"]
    sasd_attn_mask = data["sasd_attn_mask"]
    sasd_inputs = data["inputs"]  # already [2B, 2L]
    sasd_positions = data["inputs_position"]  # dummy [2B, 2L]
    sasd_attention_metadata = {
        "sasd_rope_cos": sasd_cos,
        "sasd_rope_sin": sasd_sin,
        "sasd_attn_mask": sasd_attn_mask,
    }
    # Frozen-ViT image embeds (optional): when the SASD batch carries precomputed image
    # embeds + their IMAGE_TOK positions, thread them so _apply_embedding scatters the
    # frozen ViT embeds at the image-token positions in the doubled sequence (NNX
    # per_sample_loss_sums embed_row's .at[img_pos].set). Gated on presence -> text-only
    # SASD (no images) is unaffected.
  ...
  if is_mdlm:
    ...
    rng_mdlm, dropout_rng = jax.random.split(dropout_rng)
    rng_t, rng_mask = jax.random.split(rng_mdlm)
    B = data["inputs"].shape[0]
    mdlm_t = jax.random.uniform(rng_t, (B,), minval=1e-3, maxval=1.0)
    mdlm_x_t, _ = forward_mask(data["inputs"], mdlm_t,
                               int(config.mask_token_id), rng_mask)
    mdlm_x_0 = data["inputs"]            # save the clean targets before overwrite
    data["inputs"] = mdlm_x_t            # model now sees noised inputs
  # decimate proportion of data when per_device_batch_size<1.
  # SASD batches carry higher-rank keys (cos/sin/attn_mask/labels_final/...) that
  # the generic rank-2 slicing would crash on; the SASD branch handles its own
  # (already-sized) batch and runs a dedicated model.apply, so skip this loop.
  if not is_sasd:
    if is_train:
  ...
  if isinstance(model, nn.Module):
    ...
    if sparsity_enabled:
      ...
      model_vars = params
    if is_sasd:
      # SASD forward: doubled [2B, 2L] inputs, NO segment ids (the hybrid 4D mask
      # fully defines attention), 3D M-RoPE cos/sin + hybrid mask via attention_metadata.
      logits, intermediate_outputs = model.apply(
          ...
          attention_metadata=sasd_attention_metadata,
      )
    else:
      logits, intermediate_outputs = model.apply(
          model_vars,
    ...
    if is_sasd:
      # SASD loss: section_weighted_ce(noisy half, both rows) + causal_ce(clean half,
      # mdm row). sasd_loss_from_logits reproduces train_tpu.per_sample_loss_sums +
      # the global DP-correct normaliser. We push the GLOBAL numerator (sec+causal sum)
      # into xent_sum and total_weights = sum_b(num_items) so the downstream
      # loss = xent_sum / (total_weights + EPS) == the NNX loss to working precision.
      sasd_loss_val, sasd_sec_sum, sasd_cau_sum, sasd_total_weights = sasd_loss_from_logits(
          logits, sasd_labels_final, sasd_original_labels, sasd_weights,
          sasd_num_items, sasd_B, sasd_L,
      )
      total_z_loss = jnp.zeros((), dtype=jnp.float32)
      xent_sum = sasd_sec_sum + sasd_cau_sum
      total_weights_for_sasd = sasd_total_weights
      ...
      # Phase 23 — MDLM loss path. The model has already produced logits
      # given the noised inputs; compute the Sahoo 2024 weighted CE on
      # masked positions and stuff it into `xent_sum` so the downstream
      # division by `total_weights` produces the right scalar.
      mdlm_l = mdlm_loss(
          logits, mdlm_x_0, data["inputs"], mdlm_t,
          int(config.mask_token_id),
          int(getattr(config, "pad_token_id", 0)),
      )
      total_z_loss = jnp.zeros((), dtype=jnp.float32)
      total_weights_for_mdlm = jnp.sum(data["targets_segmentation"] != 0)
      xent_sum = mdlm_l * total_weights_for_mdlm
    else:
  ...
  if is_sasd:
    # SASD's global denominator is sum_b(num_items) (2 * total response tokens),
    # NOT a segment-id count; data has no targets_segmentation under this objective.
    total_weights = total_weights_for_sasd
  else:
