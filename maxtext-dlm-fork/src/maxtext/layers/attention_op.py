...
class AttentionOp(nnx.Module):
  ...
  def apply_attention(
      ...
      sasd_attn_mask: Array | None = None,
      *,
  ...
  def apply_attention_dot(
    ...
    if sasd_attn_mask is not None:
      # SASD (Fast-dDrive) hybrid block-causal mask injected host-side. Bypass
      # generate_attention_mask entirely and reproduce the NNX masking exactly:
      #   logits = where(mask4d, logits, float32.min)  (NNX qwen2_5_text LARGE_NEGATIVE).
      # sasd_attn_mask: bool [b, q_len, kv_len] (True=attend). attn_weights:
      # [b, n_kv, g, q_len, kv_len]; insert the (1,1) head/group axes to broadcast.
      sasd_mask_bool = sasd_attn_mask[:, None, None, :, :]
      large_neg = jnp.finfo(attn_weights.dtype).min
      attn_weights = jnp.where(sasd_mask_bool, attn_weights, large_neg)
      attn_mask = None
    else:
  ...
  def __call__(
      ...
      record_max_logits: bool = False,
      sasd_attn_mask: Optional[Array] = None,
  ):
