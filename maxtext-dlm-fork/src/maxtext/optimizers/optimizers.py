...
def get_optimizer(config, learning_rate_schedule, model=None):
  ...
  if config.opt_type == "adamw":
    ...
    # Adafactor: factored 2nd-moment estimate -> NO large fp32 adam moments. Required for
    # SASD fine-tune of the Fast-dDrive 3B on a single GPU (adamw's fp32 mu/nu OOM at ~24GB);
    # on a TPU pod where FSDP shards the optimizer state, adamw is fine and preferred. Mirrors
    # the hand-built optax.adafactor in diffusion/tests/sasd_lossdecrease_test.py (verified
    # loss 0.98 -> 0.64 on the 5090).
    base_opt = optax.adafactor(
        learning_rate=learning_rate_schedule,
        multiply_by_parameter_scale=bool(getattr(config, "adafactor_multiply_by_parameter_scale", True)),
        min_dim_size_to_factor=int(getattr(config, "adafactor_min_dim_size_to_factor", 128)),
    )
