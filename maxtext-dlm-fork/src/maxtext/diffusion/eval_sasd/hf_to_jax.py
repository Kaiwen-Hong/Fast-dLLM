...
"""
# ── VENDORED from Fast-dLLM ddrive_jax/convert/hf_to_jax.py @ 4b0f4f2 — DO NOT EDIT logic here.
# Self-contained SASD inference for the internal-TPU fork (B2). Changes vs source: (1) imports
# rewritten ddrive_jax.* -> local eval_sasd.*; (2) bf16-hardened tensor reads (_st_tensor_f32),
# because the B1-exported snapshot is bf16 and safetensors' numpy framework cannot decode bf16.
...
def _st_tensor_f32(path, key):
    """Read one safetensors tensor as fp32 numpy, tolerant of BF16/F16/F32. The B1-exported
    snapshot is bf16 and safetensors' numpy framework cannot decode bf16. Bit-identical to
    ``f.get_tensor(key).astype(np.float32)`` for F32/F16 sources; downstream ``_set`` casts to
    the model leaf dtype, so reading fp32 then casting is correct for any cfg dtype."""
...
def _load_all_tensors(snapshot_dir: str) -> dict[str, np.ndarray]:
    ...
    for fname in sorted(os.listdir(snapshot_dir)):
        ...
        for k in keys:
            out[k] = _st_tensor_f32(path, k)   # BF16/F16/F32 -> fp32
    return out
    ...
    for fname in sorted(os.listdir(snapshot_dir)):
        ...
        for k in keys:
            ...
            if k.startswith("visual."):
                vit_keys += 1
            if k == "lm_head.weight":
                lm_head_b = _st_tensor_f32(sf_path, k)
                continue
            if k not in want:
                continue
            path, tr = want[k]
            arr = _st_tensor_f32(sf_path, k)   # BF16/F16/F32 -> fp32 (B1 export is bf16)
