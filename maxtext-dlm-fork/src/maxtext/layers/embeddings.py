...
def attend_on_embedding(
  ...
  # The logit-projection input dtype follows attend_dtype so that
  # config.logits_dot_in_fp32=True yields a genuine fp32 table*hidden product
  # (matching e.g. the Fast-dDrive NNX tied `embed_tokens.attend`). Otherwise we
  # keep the historical bf16 cast for throughput.
  table_dtype = jnp.float32 if attend_dtype == jnp.float32 else jnp.bfloat16
  return jnp.dot(
      query,
      jnp.asarray(embedding_table, table_dtype).T,
