...
| 文件 | 职责 | 关键符号 |
|---|---|---|
| `src/maxtext/diffusion/sasd.py` | SASD 数学（loss/noising/mask/M-RoPE/embed-doubling/host prep/global loss） | `make_batch`、`num_items`(2×)、`mrope_cos_sin`(float64 contiguous)、`compute_fast_ddrive_image_embeds`、`prepare_sasd_inputs`(`flat=2b+r`)、`sasd_loss_from_logits`；`_ce_per_token`(fp32 per-token NLL，**[订正 2026-06-21] 现为 VOCAB-TILED online-logsumexp CE**：对 V 维分 `_CE_VOCAB_TILES=8` 块做 streaming logsumexp，fp32 峰值锁在 `[N,V_tile]` 而非 `[N,V]`，**无 `jax.lax.map`、无 per-row checkpoint**；配 bf16 logits 修 720-OOM——真 v5e 编译+step0 通过 `TPU_OOM: SOLVED`，loss-decrease pending。取代了 2026-06-20 chunked-CE（TPU 回归 87.25G）与中间的 fused logsumexp CE（17.11G floor、必要但不充分）；数学等价。根因/数字见 [`02_gotchas.md`](02_gotchas.md) + [`../1plans/07_fidelity_fixes_2026-06-20.md`](../1plans/07_fidelity_fixes_2026-06-20.md)) |
