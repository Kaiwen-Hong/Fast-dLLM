...
- **train-step 显存的 DOMINANT 项是 loss 侧的 fp32 全词表 CE，不是 ViT 激活。** CE 在 causal-shift + 2L packing 后展平成 `[N,V]`（`N=2L`，`V=151936`），fp32 全词表 transient 随 **2L 线性增长**：168 配置 `N=2L=1856` 装得下 v5e；720 配置 `N=2L=3712` 翻倍 → 在 v5e 单芯 OOM（这正是「168 装得下 v5e 而 720 OOM」的机制）。`sasd.py`。**[订正 2026-06-21]** 旧表述「修复 = chunked CE（`jax.lax.map(batch_size=512)`+`@jax.checkpoint`，12.66G proxy，真 v5e pending）」现已**作废**——见下条。
- **[订正 2026-06-21 续 → OOM SOLVED] 修法 = vocab-tiled online-logsumexp `_ce_per_token` + bf16 logits（不是 chunked-CE、也不是光 fused CE）。`TPU_OOM: SOLVED; LOSS_DECREASE: pending`。** 演化轨迹：(1) chunked-CE（`jax.lax.map(batch_size=512)`+per-row `@jax.checkpoint`）是 TPU 灾难性回归（GPU 19.6→13.6G 好看，真 v5e HLO temporaries **17.11G→87.25G** `RESOURCE_EXHAUSTED`——`lax.map`+per-row checkpoint 在 TPU lower 成病态、保留 ≈22 份 fp32 `[~5565,V]`≈+70G，GPU 的 `hlo_rematerialization` 救得了、TPU 不同 pass 救不了；证据 `/tmp/sanity_fix/tpu_720_confirm.log`）。(2) fused logsumexp CE（`logsumexp(logits.f32)-logits.f32[target]`）**移除了回归但回到 17.11G floor**（真 v5e `TPU_VALIDATION: PARTIAL`，仍超 15.75G/chip 预算 1.36G，原始 `bnu1tt5e9` floor 未破，`sasd_vit_remat=on` 对此数字 ~0 效果——floor 在 loss 侧 fp32 `[N,V]`+logits，**不是** ViT；证据 `/tmp/sanity_fix/tpu_720_confirm2.log`）。(3) **实际修复 = vocab-tiled CE**：`_ce_per_token` 改为对 V 维分 `_CE_VOCAB_TILES = 8` 块做 **online/streaming logsumexp**（逐块累积 running max `m` + running sum `s`，`m+log(s)-tgt == -log_softmax[target]`），fp32 峰值锁在 `[N, V_tile]` 而非 `[N,V]`，**无 `jax.lax.map`、无 per-row `jax.checkpoint`**（普通 Python for 静态展开，XLA 友好；不依赖 XLA fusion 运气），签名 `_ce_per_token(logits2d, labels1d, ignore=-100)` 与 `(nll, valid)` 契约不变。配套 **bf16 logits**（启动加 `logits_dot_in_fp32=false cast_logits_to_fp32=false`，让 `[2B,2L,V]` 那份也是 bf16）。GPU loss-decrease 复验 PASS（`0.981→0.636` `SASD_TRAIN_LOSSDECREASE_PASS`，全 finite，peak 18.54G，与 fused 0.983→0.637 / chunked 0.981→0.637 实质一致＝数学不变；证据 `/tmp/sanity_fix/gpu_lossdecrease_vtile.log`）。**真 v5e-16 复跑（run `vit720conf`，bundle `fastddrive-20260621_065405-d627036-dirty`）= OOM 已解决**：train step **编译通过、无 `RESOURCE_EXHAUSTED`**（正是 chunked 87.25G / fused 17.11G / 原始 17.11G 三次全卡死的那一步，这次过了），**step 0 已执行**（日志到 `checkpointing.py:841 Waiting for step 0 to finish before checkpoint`，在 train loop 内 train_step 之后）——vocab-tiled CE + bf16 logits 把 720 trainable step 装进 v5e 单芯 15.75G。**⏳ 仍 pending**：`completed step: N, loss:` 的下降数字（在 step-0 GCS 存档后才打印，run 在飞）——**不得声称训练已完整验证**。证据 `/tmp/sanity_fix/tpu_720_vtile.log`。配套 FIX（沿用）：SASD 不再把 logits 塞进 `intermediate_outputs`（`train.py:382`）、`sasd_vit_remat` 默认 True、AdamW `mu_dtype: float32`。`sasd.py:_ce_per_token`（`_CE_VOCAB_TILES=8`）。canonical 数字 + 链路见 [`../1plans/07_fidelity_fixes_2026-06-20.md`](../1plans/07_fidelity_fixes_2026-06-20.md) §6.3 / §7 与 [`../8doc_updates/2026-06-21_sasd-tpu-oom-fix.md`](../8doc_updates/2026-06-21_sasd-tpu-oom-fix.md) §5.2。
- **[NO-GO 教训 2026-06-21] bf16-logits-ALONE（光关 fp32-logit flags、不动 CE）净省 ~0，不是 1.36G。** 关 `logits_dot_in_fp32`+`cast_logits_to_fp32` 让全量 `[2B,2L,V]` logits 从 fp32 变 bf16（4.2→2.1G），**但** `_ce_per_token` 紧接着 `.astype(fp32)` 对 shifted slice `[2,1855,V]` 又重新 materialize 一份 2.1G fp32 `[N,V]` → 抵消，**净省 ~0.001G**；只有当 CE 本身 vocab-tiled（fp32 峰值锁在 `[N,V_tile]`）时 bf16 logits 才真正生效。另注:**`float32_logits` 是注意力 softmax 精度，不是词表 logits 标志**——别混。证据 `/tmp/review2/decision_review.out`。背景:大词表 CE 显存爆炸是 LLM 训练教科书问题,vocab-tiling 是头号标准解（MaxText 内置 `num_vocab_tiling`+`logits_from_hidden_states`；T5X/PaxML/Levanter 同；SOTA 变体 Cut Cross-Entropy）——SASD 的特殊点只是自定义 loss 路径绕过了内置 tiling。
- **[KEY LESSON 2026-06-21] GPU 显存 proxy（BFC `MaxInUse`）对 TPU 是 necessary-NOT-sufficient，甚至会 MISLEAD。** XLA 对**同一张图**在 GPU/TPU 上 lower 方式差异巨大（GPU 有 `hlo_rematerialization` pass、TPU 不同）→ **只在 GPU 验过的显存优化必须在真 TPU 复核**，不得据 GPU proxy 单独下「fits v5e」结论。chunked-CE 就是反例：GPU 好（19.6→13.6G），真 v5e 灾难（17→87G）。下条（静态 `Total memory size` 误导）讲的是 MaxText 静态指标无预测力；本条讲的是连**动态 BFC proxy** 也只在 GPU 维度可信、跨到 TPU 会骗人。
- **诊断技巧：MaxText 的静态 `Total memory size` 指标（`utils/max_utils.py:796`）查不出这类变化、是误导的 OOM 预测器。** 它在 baseline/ViT-remat/chunked-CE/bf16-logit 之间几乎不动（Temp 13.8↔13.6G），且在 32G GPU 上**永不触发** OOM → 对 16G v5e 毫无预测力。次优信号是 BFC `MaxInUse`（用 `XLA_PYTHON_CLIENT_MEM_FRACTION=0.50` 把 GPU 压到 ~15.7G 当 v5e proxy 再读 `bfc_allocator.cc` 的 `MaxInUse`）——但**[订正 2026-06-21] 连 BFC proxy 跨到 TPU 也会骗人**（见上方 KEY LESSON）：旧 session 据此读出「720 trainable step proxy 峰值 ~12.66G < 15.75G v5e 预算 → fits v5e；真 v5e 复跑 pending」的结论，**已被 2026-06-21 真 v5e-16 复跑（run `vit720conf`）证伪**——chunked-CE bundle 实测 **HLO temporaries 87.25G ≫ 15.75G、`RESOURCE_EXHAUSTED`**（GPU proxy 假象）。**本条 load-bearing 教训不变**：只在 GPU 验过的显存优化必须在真 TPU 复核。**[订正 2026-06-21 续 → OOM SOLVED] 修法演化的最终结论**：fused CE 把 `87.25G → 17.11G`（移除回归、退回 plain 基线）但仍超预算 1.36G（`TPU_VALIDATION: PARTIAL`，必要但不充分）；**真正的修法 = vocab-tiled online-logsumexp CE（`_CE_VOCAB_TILES=8`）+ bf16 logits**——真 v5e-16 复跑 **编译通过、无 `RESOURCE_EXHAUSTED`、step 0 已跑 ＝ `TPU_OOM: SOLVED`**；loss-decrease 数字 pending（run in flight，`LOSS_DECREASE: pending`，**未声称训练已完整验证**）。详见本节上方两条与 [`../8doc_updates/2026-06-21_sasd-tpu-oom-fix.md`](../8doc_updates/2026-06-21_sasd-tpu-oom-fix.md) §5.2。链路与所有 dead-end（ViT-remat / 单独 `cast_logits_to_fp32=False` / `enable_checkpointing=false`）见 [`../1plans/07_fidelity_fixes_2026-06-20.md`](../1plans/07_fidelity_fixes_2026-06-20.md) §6.3 / §7。
...
> **[2026-06-21] 两个训练 config（保真审计后拆分）**：
> - `configs/sasd_waymo.yml` = **SANITY / smoke**（单卡快验）：退役 168 shape（`sasd_seq_len 1184`/`sasd_num_image_tokens 336`）、frozen ViT、adafactor、lr 1e-4、cosine。**真跑别直接用**。
> - `configs/sasd_waymo_canonical.yml` = **canonical 大规模**（保真）：720 shape（`1856`/`1440`，`max_target_length 3712`）、**trainable ViT**、AdamW（`adam_b2 0.999`/`weight_decay 0.0` 对齐 HF；MaxText 默认 0.95/0.1 会 drift）、lr **1e-5**、constant_with_warmup（warmup 0.03、`learning_rate_final_fraction 1.0`）、`gradient_accumulation_steps 4`。`steps` launch 时按 2-epoch 设。镜像 `fast_ddrive/train_scripts/train_waymo_sasd.sh`。
