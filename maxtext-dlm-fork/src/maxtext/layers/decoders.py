...
class Decoder(nn.Module):
  ...
  def _apply_embedding(
    ...
    # SASD (Fast-dDrive) frozen-ViT image-embed injection: when attention_metadata
    # carries precomputed image embeds + their IMAGE_TOK positions, scatter the embeds
    # into the token-embedding rows at those positions -- the literal vectorized form
    # of NNX per_sample_loss_sums' ``embed_tokens(ifn[r]).at[img_pos].set(image_embeds)``
    # (sasd_image_embeds [2B, 2N, D], sasd_image_pos [2B, 2N], same positions/embeds for
    # both doubled rows). Done on the RAW token embeds before dropout/dtype-cast, exactly
  ...
  def apply_output_head(self, shared_embedding: nn.Module | nnx.Module, y, deterministic, model_mode):
    ...
    # Opt-in: expose the post-final-norm hidden (pre-logit-projection) for parity
    # tests. Inert unless config.sow_normed_hidden is set true.
    if getattr(cfg, "sow_normed_hidden", False):
      self.sow("intermediates", "normed_hidden", y)
