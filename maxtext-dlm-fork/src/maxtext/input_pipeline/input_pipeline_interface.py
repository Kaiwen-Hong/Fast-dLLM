...
def create_data_iterator(config: pyconfig.HyperParameters, mesh):
  ...
  # Waymo SASD (Fast-dDrive) path: a custom iterator yields the doubled [2B,2L] SASD batch
  # dict (inputs + sasd_* keys + optional frozen-ViT image embeds) the objective="sasd"
  # loss branch consumes. The generic tfds/grain/hf paths and get_input_data_sharding flow
  # assume rank-2 [B,L] tensors with max_target_length shapes and would break on the
  # higher-rank SASD keys, so this is a dedicated iterator (mirrors SyntheticDataIterator's
  # role). The loader already shards per host via process_index/process_count.
  if config.dataset_type == "waymo_sasd":
    from maxtext.input_pipeline.waymo_sasd_data_processing import WaymoSasdDataIterator

    train_iterator = WaymoSasdDataIterator(config, mesh)
    eval_iterator = WaymoSasdDataIterator(config, mesh) if config.eval_interval > 0 else None
