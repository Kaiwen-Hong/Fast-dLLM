...
STEP 3(`sanitycheck_dataprocessing.md`)+ STEP I(`../7for_internal_inference/00_run_inference.md`)需要这几样在本地(5090)产出、ship 到 GCS 的东西:快照/val 子集给两者;GT pkl + **预烤的 `prep_val_full`** 给 STEP I 的官方 metric(让内部 Track 2 **零 proto/tf/torch**:prep_val 与 ViT 无关,metric 用 `--gt pkl` 走纯 numpy)。代码侧
