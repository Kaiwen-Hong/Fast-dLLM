...
| 6 | batch>1 vision merge reshape | **PARTIAL — merge reshape ADOPTED but NOT sufficient.** Training at B=2 fails BEFORE the merge: `@_jax_utils.flatten_unflatten_batch_dim()` on the gemma forward validates every arg's leading batch dim, and the packed PVI `[1, B·n, D]` conflicts with tokens `[B, L]` ("Inconsistent batch shape for 'images.patches'"). This is what internal patch #4 (tree-map shielding of `images`) was actually FOR — my row-9 "fix the annotation instead" verdict addressed ktyping but not this wrapper. Overnight runs use **batch=1 native × accum=8 (global 8, criterion-compliant)**. OPEN ITEM: shield/whitelist `images` in the batch-dim wrapper (or pack PVI per-row) to unlock native B>1 |
