...
def get_shaped_batch(config):
  ...
  # SASD objective: the grain loader yields a DOUBLED [2B, 2L] batch plus higher-rank
  # sasd_* metadata keys (cos/sin/mask/labels/...) and (optionally) the frozen-ViT image
  # path. The standard rank-2 AR keys below do NOT match, so the standard
  # `p_train_step.lower(state, shaped_batch, init_rng)` would mismatch shapes/keys at
  # compile. Build the exact abstract dict the objective=="sasd" loss_fn branch reads.
  if getattr(config, "objective", "ar") == "sasd":
    gbs = config.global_batch_size_to_load            # B (== per_device_batch * devices)
    L = int(config.sasd_seq_len)
    assert L > 0, "objective=sasd with dataset_type=waymo_sasd requires sasd_seq_len>0 in the config."
    twoB = 2 * gbs
    twoL = 2 * L
    hd = config.head_dim
    V = config.vocab_size  # noqa: F841  (kept for documentation; logits, not a batch key)
    shaped = {
        "inputs": jax.ShapeDtypeStruct((twoB, twoL), jnp.int32),
        "inputs_position": jax.ShapeDtypeStruct((twoB, twoL), jnp.int32),
        "sasd_cos": jax.ShapeDtypeStruct((twoB, twoL, hd), jnp.float32),
        "sasd_sin": jax.ShapeDtypeStruct((twoB, twoL, hd), jnp.float32),
        "sasd_attn_mask": jax.ShapeDtypeStruct((twoB, twoL, twoL), jnp.bool_),
        "sasd_labels_final": jax.ShapeDtypeStruct((gbs, 2, L), jnp.int32),
        "sasd_original_labels": jax.ShapeDtypeStruct((gbs, 1, L), jnp.int32),
        "sasd_weights": jax.ShapeDtypeStruct((gbs, 2, L), jnp.float32),
        "sasd_num_items": jax.ShapeDtypeStruct((gbs,), jnp.float32),
    }
...
def get_abstract_state(config, mesh, init_state_fn, is_training=True):
  ...
  if is_training and config.opt_type == "adafactor":
    # Adafactor keeps FACTORED second-moment estimates: v_row / v_col are rank (n-1) views
    # of an n-D param, so they cannot inherit the param's full n-axis NamedSharding spec
    # (`logical_to_mesh_sharding` derives the opt_state spec from the param's logical
    # annotation). Applying a 2-axis spec to a rank-1 factored moment raises
    # "Sharding ... is only valid for values of rank at least 2". Sanitize: for any opt_state
    # leaf whose array rank is < the spec's axis count, replicate it (a fully-replicated
    # rank-matched spec). Factored moments are tiny, so replicating them is harmless on a
    # pod; on a single host every mesh axis is size 1 so this is a pure no-op rename. adamw
    # (param-shaped moments) is untouched.
    abstract_opt_state = max_utils.unbox_logicallypartioned(abstract_state).opt_state

    def _sanitize_opt_sharding(leaf, shard):
      spec = getattr(shard, "spec", None)
      if spec is None or not hasattr(leaf, "ndim"):
        return shard
      if len(spec) > leaf.ndim:
        return jax.sharding.NamedSharding(mesh, jax.sharding.PartitionSpec(*([None] * leaf.ndim)))
      return shard

    state_mesh_shardings = state_mesh_shardings.replace(
        opt_state=jax.tree.map(
            _sanitize_opt_sharding,
            abstract_opt_state,
