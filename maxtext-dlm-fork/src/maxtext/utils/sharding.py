...
def get_input_data_sharding(config, mesh):
  ...
  if getattr(config, "objective", "ar") == "sasd" and config.dataset_type == "waymo_sasd":
    return _get_sasd_input_data_sharding(config, mesh)
...
def _get_sasd_input_data_sharding(config, mesh):
  """Per-key data sharding for the SASD batch (dataset_type='waymo_sasd').

  The standard path returns ONE 2-axis NamedSharding broadcast to every leaf, which is
  invalid for the higher- and lower-rank SASD keys (e.g. rank-1 sasd_num_items, rank-3
  sasd_attn_mask). Build a pytree of per-key NamedShardings: shard ONLY the batch axis
  (axis 0) by the data/fsdp batch mesh axes and replicate every other axis. All SASD keys
  are batch-major on axis 0 (2B for the doubled rows, B for the [B,..] keys), so axis-0
  sharding is consistent; the structure matches the dict the WaymoSasdDataIterator yields and
  get_shaped_batch builds, so it can be used both as `jit` in_shardings and in `device_put`.
  """
  # Mesh axes that carry the batch (axis 0): the first logical input-data axis ->
  # P(<mesh axes>). Trailing axes replicated (None).
  batch_logical = config.input_data_sharding_logical_axes[0]
  batch_mesh_spec = logical_to_mesh_axes([batch_logical], mesh, rules=config.logical_axis_rules)
  batch_axis = batch_mesh_spec[0]  # the mesh axis tuple/name (or None)

  def _sharding_for_rank(rank):
    spec = [batch_axis] + [None] * (rank - 1)
    return NamedSharding(mesh, P(*spec))

  B = config.global_batch_size_to_load
  L = int(config.sasd_seq_len)
  N = int(config.sasd_num_image_tokens)
  twoB, twoL = 2 * B, 2 * L  # noqa: F841 (documentation of the expected shapes)

  out = {
      "inputs": _sharding_for_rank(2),               # [2B, 2L]
      "inputs_position": _sharding_for_rank(2),       # [2B, 2L]
      "sasd_cos": _sharding_for_rank(3),              # [2B, 2L, hd]
      "sasd_sin": _sharding_for_rank(3),              # [2B, 2L, hd]
      "sasd_attn_mask": _sharding_for_rank(3),        # [2B, 2L, 2L]
      "sasd_labels_final": _sharding_for_rank(3),     # [B, 2, L]
      "sasd_original_labels": _sharding_for_rank(3),  # [B, 1, L]
      "sasd_weights": _sharding_for_rank(3),          # [B, 2, L]
