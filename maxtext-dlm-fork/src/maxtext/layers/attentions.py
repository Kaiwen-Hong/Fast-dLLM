...
class Attention(nnx.Module):
  ...
  def __call__(
    ...
    # SASD (Fast-dDrive) 3D M-RoPE injection: when attention_metadata carries
    # precomputed contiguous-chunk cos/sin (sasd_rope_cos/sasd_rope_sin, shape
    # [batch, seq, head_dim]), bypass MaxText's RoPE entirely and apply the HF
    # full-form rotation (q*cos + rotate_half(q)*sin) in fp32 -- bit-identical to
    # the NNX hidden_forward_mrope_cs path. config.use_mrope MUST be false so
    # MaxText's interleaved M-RoPE class is never built. See maxtext.diffusion.sasd.
    sasd_cos = attention_metadata.get("sasd_rope_cos") if attention_metadata else None
    sasd_sin = attention_metadata.get("sasd_rope_sin") if attention_metadata else None
    if use_rope and sasd_cos is not None:
      from maxtext.diffusion.sasd import apply_rope_full as _sasd_apply_rope_full

      query = _sasd_apply_rope_full(query, sasd_cos, sasd_sin)
      key = _sasd_apply_rope_full(key, sasd_cos, sasd_sin)
    elif use_rope:
    ...
    if self.config.attention == "paged" and model_mode != MODEL_MODE_TRAIN:
      ...
      # SASD: precomputed hybrid block-causal 4D mask injected via attention_metadata.
      sasd_attn_mask = attention_metadata.get("sasd_attn_mask") if attention_metadata else None
