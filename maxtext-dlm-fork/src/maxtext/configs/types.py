...
class DiffusionObjective(BaseModel):
  """Training objective selection for the MaxText-DLM (block-diffusion) fork.

  'ar' keeps the original autoregressive cross-entropy. 'mdlm' replaces inputs
  with a forward-masked corruption before model.apply and computes a weighted CE
  (see `maxtext.diffusion` / trainers/pre_train/train.py). See base.yml for usage.
  """

  objective: Literal["ar", "mdlm", "sasd"] = Field(
      "ar",
      description=(
          "Training objective: 'ar' (autoregressive CE), 'mdlm' (masked block-diffusion CE), "
          "or 'sasd' (Fast-dDrive Section-Aware Stochastic Diffusion: doubled-sequence 3D M-RoPE "
          "hybrid block-causal forward + section_weighted_ce + causal_ce). See maxtext.diffusion.sasd."
      ),
  )
  mask_token_id: int = Field(
      -1,
      description="Token id used to corrupt inputs under the mdlm objective. Required when objective='mdlm'.",
  )
  pad_token_id: int = Field(0, description="Padding token id.")
  # --- SASD (objective='sasd') M-RoPE config -----------------------------------------
  # The Fast-dDrive 3D M-RoPE uses CONTIGUOUS-CHUNK section mixing ([T..H..W..]), NOT
  # MaxText's interleaved use_mrope path ([THTW..]). For SASD we compute cos/sin host-side
  # with ddrive_jax.models.qwen2_5_text.mrope_cos_sin and inject them via attention_metadata
  # (use_mrope MUST stay false). sasd_mrope_section is (temporal, height, width); 2*sum == head_dim.
  sasd_mrope_section: list[int] = Field(
      [4, 6, 6],
      description="Fast-dDrive 3D M-RoPE (temporal,height,width) chunk sizes; 2*sum == head_dim.",
  )
  sasd_rope_theta: float = Field(
      1_000_000.0,
      description="RoPE theta for the SASD 3D M-RoPE (Fast-dDrive qwen2.5 = 1e6).",
  )
  # --- SASD data-path shapes (dataset_type='waymo_sasd') -------------------------------
  # The grain SASD loader yields a DOUBLED [2B, 2L] batch. These describe the per-sample
  # single (un-doubled) sequence length L and per-sample image-token count N so the
  # data iterator + get_shaped_batch can build exact static shapes (2L, 2N) WITHOUT
  # relying on max_target_length (which only needs to be >= 2L). Set from the dataset
...
class Logits(BaseModel):
  ...
  sow_normed_hidden: bool = Field(
      False,
      description="If true, sow the post-final-norm hidden (pre-logit-projection) into "
      "intermediates['normed_hidden'] for parity/debug inspection. Inert in production.",
  )
...
class Optimizer(BaseModel):
  ...
  adafactor_multiply_by_parameter_scale: bool = Field(
      True, description="Adafactor: scale the learning rate by the RMS of each parameter (relative step size)."
  )
  adafactor_min_dim_size_to_factor: PositiveInt = Field(
      128, description="Adafactor: only factor the second moment for dimensions >= this size."
  )
